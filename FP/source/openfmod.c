...
#define INPUT_TXT // O ARGS
#define BEAM_TYPE 3//RAND_ANG 1 || DEB_ANG 2|| PARALLEL_BEAM 3
#define PROJOBJ 2// CIRCLE 1 || SQUARE 2 || CIRCLE_IN_SQUARE 3 || PHANTOM 4
#define DOSPI  //ABANOICO O DOSPI
...
int main(int argc, char **argv)
{
	...
	double angulos[5]={ -30*factorcon, -15*factorcon, 0*factorcon, 15*factorcon, 30*factorcon};
	finp=fopen("input/AngulosBundle.txt", "w");
	...
	fclose(finp);
	finp=fopen("input/NumAngAbanico.txt", "w");
	fprintf(finp, "%d\n", NumAdq);
	fclose(finp);
	#endif
	

	//PHANTOM PARAMETERS Can be found those of typical Shepp-Logan phantom @ Kak & Sleaney p.102
	float x0[5]={2, 1, -2, 10, 0}; // Center of ellipsis x-axis

	float y0[5] ={2, 1, 2, -10, 0}; //Center of ellipsis y-axis

	float ax[5]={2.1, 20, 10, 3, 40}; // Semi-axis

	float bx[5]={1, 30, 7, 13, 20}; // Semiaxis

	float val[5]={1.0, 1.5, 2.3, 1.44, 1.2}; //Value of the ellipsis
...
switch(BEAM_TYPE){
	...
	*pos0 = factor*anchura/2-deltadis+ 2*deltadis*num_rand; /*Fuente extensa, posicion y aleatoria*/
	 *(pos0+1)= betarnd;
	 ...
	 case(2): // DEB_ANG /*angulos equiespaciados en el cono de posibles angs*/
