...
__global__ void cu_po_initialize_coef_flo_2pi(int NumPix, int NumAng, float factordim,int limy, int dimx, int dimy, int dimtot, int NRay, float deltaang, float factorpos, float *coe){
	...
	int  cont3, pixx, pixy, c1, c2a, c2b, c2c, c2d, c1max;
	float cosbeta, sinbeta, beta, deltax, deltay, posx, posy, posx2, posy2;
	int cont=blockIdx.x*blockDim.x+threadIdx.x;
	int cont2=blockIdx.y*blockDim.y+threadIdx.y;
	//~ c1=cont*NumPix+cont2;
	c1=cont*dimtot+cont2;
	//~ #ifdef PARALLEL_BEAM
	//~ #pragma omp parallel 
	//~ {
		//~ #pragma omp for private(cont2, cont3, pixx, pixy, cosbeta, sinbeta, beta, deltax, deltay, posx, posy, posx2, posy2)
...
__global__ void cu_pa_initialize_coef_flo_2pi(int NumPix, int NumAng, float factordim,  int limy, int dimx, int dimy, int dimtot, int NRay, float deltaang, float factorpos, float *coe){
	...
	//AtomicAdd solo funciona con int val
		int  cont3, pixx, pixy, c1, c2a, c2b, c2c, c2d, c1max;
		float cosbeta, sinbeta, beta, deltax, deltay, posx, posy, posx2, posy2;
		int cont=blockIdx.x*blockDim.x+threadIdx.x;
		int cont2=blockIdx.y*blockDim.y+threadIdx.y;
		c1=cont*(NumAng+1)+cont2;
				//Comenzar parte paralela
				beta = cont*deltaang;
				float factora=1000;
				//~ #ifdef ABANICO
				//~ beta=angulos[cont];
				...
				//~ printf("beta = %f, dimx, dimemsiones = %d %d, limy = %d, NumPix %d\n", beta, dimx, dimy, limy, NumPix);
					...
					for(cont3=0; cont3<dimx; cont3++)
					{
					    ...
					    //~ coe[c1*dimtot+c2c]+=(1-deltay+deltax)/factora;
					    //~ coe[c1*dimtot+c2d]+=(deltax+deltay)/factora;
					    atomicAdd(&coe[c1*dimtot+c2a], (2-deltax-deltay)/factora);
					    atomicAdd(&coe[c1*dimtot+c2b], (1-deltax+deltay)/factora);
					    atomicAdd(&coe[c1*dimtot+c2c], (1-deltay+deltax)/factora);
					    atomicAdd(&coe[c1*dimtot+c2d], (deltax+deltay)/factora);
					    //~ coe[c1*dimtot+c2a]=(2-deltax-deltay)/factora;				
					    //~ coe[c1*dimtot+c2b]=(1-deltax+deltay)/factora;
					    //~ coe[c1*dimtot+c2c]=(1-deltay+deltax)/factora;
					    //~ coe[c1*dimtot+c2d]=(deltax+deltay)/factora;
						...
						atomicAdd(&coe[c1max*dimtot+c2a], coe[c1*dimtot+c2a]);
					    atomicAdd(&coe[c1max*dimtot+c2b], coe[c1*dimtot+c2b]);
					    atomicAdd(&coe[c1max*dimtot+c2c], coe[c1*dimtot+c2c]);
					    atomicAdd(&coe[c1max*dimtot+c2d], coe[c1*dimtot+c2d]);
					    //~ __syncthreads();  //CHANGE 26-9
						...
						//~ cuPrintf("coe[%d] = %f\n", c1*dimtot+c2a, coe[c1*dimtot+c2a]);
...
__global__ void initialize_coe(float *coe, int TX)
{
	int cont=blockIdx.x*blockDim.x+threadIdx.x;
	int cont2 = blockIdx.y*blockDim.y+threadIdx.y;
	int cont3=cont*TX+cont2;
	coe[cont3]=0.0f;
